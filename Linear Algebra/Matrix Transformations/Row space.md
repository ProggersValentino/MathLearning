...
A row space is the [column space](obsidian://open?vault=MathLearning&file=Linear%20Algebra%2FVectors%20%26%20spaces%2FColumn%20Space) of the [transposed matrix](obsidian://open?vault=MathLearning&file=Linear%20Algebra%2FMatrix%20Transformations%2FTransposing), 
...
When it comes to determining the [rank](obsidian://open?vault=MathLearning&file=Linear%20Algebra%2FVectors%20%26%20spaces%2FColumn%20Space) of $A^T$, it follows the same process and produces the same result as $Rank(A)$ where its determined by the amount of pivot entries produced from putting matrix $A$ into [reduced row echelon form](obsidian://open?vault=MathLearning&file=Algebra%2FMatrices%2FReduced%20Row%20Echelon%20with%20Matrix%20Row%20Operations)
...
Another property with the row space is that its orthogonal to the [null space](obsidian://open?vault=MathLearning&file=Linear%20Algebra%2FVectors%20%26%20spaces%2FNull%20space) as if we visualize it
![[Pasted image 20260806134423.png]]

and apply the [dot product](obsidian://open?vault=MathLearning&file=Linear%20Algebra%2FVectors%20%26%20spaces%2FVector%20Dot%20Product%20%26%20Length) to each vector in the null space against the row space, it works out to be orthogonal
